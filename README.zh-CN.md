...
> 状态以 `feature/3.0.x` 分支上的 `2.0.x.20260630-SNAPSHOT` 为准。
...
| easy4j 依赖 | `shiro-extension-spring`、`jwt-issuer-api`（均为 `2.0.x.20260630-SNAPSHOT`） |
...
该构件尚未发布到 Maven Central。请从项目配置的制品仓库（阿里云制品仓库）获取，或从源码本地安装；`feature/3.0.x` 分支当前使用的快照版本为 `2.0.x.20260630-SNAPSHOT`。
